...
hand-rolled test keys, but the *interop* with the actual `cosign` binary
was, at the time, still an assumption. Once `cosign` (v3.1.3) became
available, a real local dry-run (`goreleaser release --snapshot --clean`
against the actual `.goreleaser.yaml`) surfaced three independent,
production-blocking bugs a real tagged release would have hit immediately:

1. **`.github/workflows/release.yml` never installed `cosign` at all.**
   The `signs:` block shells out to a `cosign` binary that was never
   present on the runner — the release would have failed at the signing
   step with "command not found." Fixed by adding
   `sigstore/cosign-installer@v3` before the GoReleaser step.
2. **`cosign sign-blob --output-signature=<path>` no longer works.**
   Confirmed against the real binary: the flag is gone in cosign v3.x —
   `sign-blob` now requires `--bundle=<path>`, which writes a JSON envelope
   (a Sigstore bundle, including a real public Rekor transparency-log
   entry) rather than a bare signature file.
3. **cosign's `--key` loader rejects plain OpenSSL-generated PEM
   keys entirely** — confirmed against the real binary with both SEC1
   ("EC PRIVATE KEY") and PKCS8 ("PRIVATE KEY") forms, both fail with
   "unsupported pem type." It only accepts a key pair generated by
   `cosign generate-key-pair`, which is always password-encrypted (needs a
   password, not just the key content). This meant the production key
   generated for the previous round's fix would never have worked with a
   real `cosign` invocation, regardless of bug #2.

**Fix:** rather than teach Pokkum's Go verifier to parse the new bundle
JSON format (real scope creep into code that was already correct and
tested), `scripts/cosign-sign-blob.sh` wraps the real `cosign` CLI: signs
with `--bundle` (the only mode that works), then extracts just
`messageSignature.signature` — confirmed to be the same base64
ASN.1-DER-over-SHA256 format the existing Go verifier already expects —
into the plain file `.goreleaser.yaml`'s `signs:` block (now pointed at
this wrapper instead of `cosign` directly) and `pokkum upgrade` both
already understand. Zero Go verification code changed. `upgrade.go`'s
embedded `DefaultReleasePublicKeyPEM` was replaced with the public half of
a properly `cosign generate-key-pair`-generated key.

**This was proven end-to-end, not just fixed and assumed working again:**
ran the real fixed pipeline locally (`release succeeded after 4m7s`,
including the real signing step via the real `cosign` binary), then
independently verified the resulting `checksums.txt.sig` against the exact
public key now embedded in `upgrade.go`, calling
`cosign.Signer.VerifyArtifactSignature` directly — the same function
`pokkum upgrade` calls in production. Passed.

**Maintainer action required:** the `COSIGN_PRIVATE_KEY` GitHub Actions
secret must be replaced (not just left as-is) with a real
`cosign generate-key-pair`-generated key, and a new `COSIGN_PASSWORD`
secret added — see [for-users.md](for-users.md).
