...
**What:** `Preflight` hard-codes a requirement for `@jesterkit/exe-sveltekit` or `@sveltejs/adapter-node`. A correctly-configured `adapter-static`-only project is rejected before `Prepare`'s own adapter check — which *is* strategy-aware — ever runs. So the static strategy cannot build a real static project at all.
