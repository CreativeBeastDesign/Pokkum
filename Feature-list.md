...
- **Hermetic Build Mode**: `--hermetic` enforces real, kernel-level network isolation on Linux (an unprivileged network namespace — no IP egress possible for the build subprocess regardless of what a compromised dependency's build-time code tries), falling back to advisory-only (`BUN_OFFLINE=1` and friends) on other platforms with a clear warning. Also strips socket-bearing environment variables (`SSH_AUTH_SOCK` and similar) from the build subprocess, closing SSH-agent-forwarding as an escape vector. Also requires cached base image resolution, pre-populated `node_modules/`, and a pre-cached Bun runtime binary (fails closed rather than downloading). Known documented residual gap: a *pathname* Unix domain socket reachable by hardcoded/conventional path, e.g. a bind-mounted `/var/run/docker.sock`, remains reachable — see `Roadmap.md`.

### OpenTelemetry & Observability
- **OpenTelemetry SDK Bootstrap** (real as of 2026-08-18 for `--strategy=exe`, see `Roadmap.md`'s PR-5 and `Vocabulary.md` §3a for the full picture): `--telemetry` compiles a real `@opentelemetry/sdk-node` NodeSDK + OTLP trace exporter bootstrap directly into the compiled entrypoint (`sveltekitutils.PrepareVirtualTelemetryEntry`, wired through `pipeline.go` → `bunexec.Compiler.Prepare`) — verified end-to-end via a real `bun build --compile` + run + a real OTLP export reaching a fake collector, not just unit-tested. **`--strategy=exe` only — no effect for the default `--strategy=layered`** (layered has no compile step to wrap; logs a build-time warning rather than silently doing nothing, tracked as a scoped follow-up). **Two further real limitations, both empirically confirmed, not assumed**: no automatic HTTP/framework instrumentation (`@opentelemetry/auto-instrumentations-node`'s module-patching approach does not take effect under Bun's runtime — real spans require the documented `hooks.server.ts` snippet in `Vocabulary.md` §3a); no metrics export (`--metrics-only` is currently non-functional — combining an OTLP metrics exporter with the SDK crashes once compiled, a real Bun bundler bug, not a Pokkum bug).
