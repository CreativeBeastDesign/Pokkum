...
- **Image Tagging (`--tag`/`-t`, `docker.tags` in `.pokkum.yaml`, `POKKUM_DOCKER_TAGS`)**: repeatable/comma-splitting, matching `--platform`'s convention. Before 2026-08-18 there was no way to tag an image at all — every build published `latest` unconditionally. Tags are applied registry-side after the image is built and hashed, so the tag set never affects the image digest. Precedence: flag > env > profile > project config > default.
...
- **Cosign & DSSE Cryptographic Signing** (real end-to-end as of 2026-08-18; before that date `--sign` validated its dependencies but never called them — see `Lessons.md`): with a signing key configured (`--signing-key`/`POKKUM_SIGNING_KEY`, PEM text or a file path, ECDSA P-256 or Ed25519), signs each pushed subject digest with Cosign (`<repo>:sha256-<hex>.sig`) and a DSSE-enveloped SLSA statement (`<repo>:sha256-<hex>.att`), dual-published to the index and every per-platform manifest, then fetches both back from the registry and cryptographically re-verifies them before the build reports success. Without a key, `--sign`'s default-on behavior pushes unsigned with a loud warning instead — `BuildResult.Signing` records that fact rather than ever claiming success; `--require-signed` turns a missing key or any self-verification failure into a hard build failure. Static-key signing only: Sigstore keyless (Fulcio/OIDC) infrastructure is used elsewhere in Pokkum for *verifying* base images and, in `pokkum verify`, verifying keyless-signed images — not for signing what Pokkum itself builds. KMS-backed signing (`awskms://` etc.) and CI OIDC identity remain open (`Roadmap.md` 2a/2b).
...
- **Secret-Inlining Guard (`secretguard`)**: Build-time regex scanner (five fixed patterns: private key headers, AWS access keys, GitHub PATs, Google API keys, generic password/secret/token assignments) — not Shannon-entropy analysis; entropy-based detection of arbitrary high-randomness strings is tracked as future work, not shipped. As of 2026-08-18, scans both pre-build source and the actual packaged build output (scoped per strategy — the whole output tree for layered, `client/`+`prerendered/` for static, asset-overlay content included; for `exe` only the pre-compile input tree, since a compiled binary can't be text-scanned), so a secret baked in by a build-time dependency or `$env/static/private` misuse is caught too, not just what shipped in your own source. A file too large or unreadable to scan fails the build (`ErrSecretScanIncomplete`) instead of silently reporting a clean pass. `--allow-secret-pattern` bypass for false positives.
