...
- **Hermetic Build Mode**: `--hermetic` enforces real, kernel-level network isolation on Linux (an unprivileged network namespace — no IP egress possible for the build subprocess regardless of what a compromised dependency's build-time code tries; a pathname Unix domain socket, e.g. a bind-mounted `/var/run/docker.sock`, is a known documented residual gap, see `Roadmap.md`), falling back to advisory-only (`BUN_OFFLINE=1` and friends) on other platforms with a clear warning. Also requires cached base image resolution, pre-populated `node_modules/`, and a pre-cached Bun runtime binary (fails closed rather than downloading).
