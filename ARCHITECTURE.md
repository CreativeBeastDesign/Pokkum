...
> [!NOTE]
> **Real as of 2026-08-18, `--strategy=exe` only** (see `Roadmap.md`'s PR-5, `Lessons.md`, and `Vocabulary.md` §3a for the full investigation). The original design below assumed SvelteKit's native `kit.experimental.tracing.server`/`instrumentation.server` flags and on-disk `src/instrumentation.server.ts` auto-loading — confirmed, by reading the real vendored `@sveltejs/kit` source, to be **on-disk-config-only with no env/CLI override**, which collides with Pokkum's own Zero-Mutation Build Sandbox invariant. The mechanism actually shipped is different (a compile-entrypoint wrapper, not a svelte.config.js/vite.config.ts transform) and is described accurately below; `internal/adapters/sveltekitutils/injector.go`'s `EnableTelemetry`/`injectExperimentalFlags`/`PrepareVirtualConfig` are **not** part of the real mechanism — they predate this fix, are never invoked by anything, and are left as dead code pending a future cleanup rather than touched as part of this fix's scope. **Scope found only after the initial fix landed**: the wrapper only takes effect through `Compile`'s `EntrypointPath` input, which the pipeline only reads for `StrategyExe` — `StrategyLayered` (Pokkum's default) never calls `Compile` at all, so requesting `--telemetry` there currently produces a build-time warning and no effect, not a silently-generated-but-unused file.

`--telemetry` compiles a real OpenTelemetry NodeSDK + OTLP trace exporter directly into the compiled entrypoint, without touching SvelteKit's own config-loading conventions at all:

* **Compile-Entrypoint Wrapper (`internal/adapters/sveltekitutils/telemetry.go`)**: `PrepareVirtualTelemetryEntry` writes two files under `<projectDir>/.pokkum/` (never the user's real project tree, and at fixed — not randomly-temp-dir'd — paths, so compiled output stays reproducible): `otel-bootstrap.ts` (`GenerateInstrumentationServer`'s real NodeSDK + OTLP trace exporter bootstrap) and `telemetry-entry.ts` (two static side-effect imports — the bootstrap first, the real adapter-generated entrypoint second; ES module ordering guarantees the SDK is fully started before the real application code runs). Skips cleanly (preserving user setup, starting no second SDK) when the project already has its own `src/instrumentation.server.{ts,js,mjs}`.
* **Wired into `bunexec.Compiler.Prepare`, `StrategyExe` only**: `req.Telemetry` (previously silently dropped by `pipeline.go`'s `PrepareRequest{}` construction — the original bug this fix closes) is checked after the real build succeeds, gated on `req.Strategy == ports.StrategyExe` specifically (a positive check, per `mem:self_review_checklist` row 11 — see the code comment for why a negative "not static" check would have silently generated a real but never-read wrapper file for `StrategyLayered`); when enabled, `PrepareResult.EntrypointPath` is swapped to the wrapper, so `Compile`'s `bun build --compile` compiles the wrapped entrypoint without needing to know telemetry exists at all. `StrategyLayered` logs a Warn explaining telemetry has no effect there yet, rather than staying silent.
