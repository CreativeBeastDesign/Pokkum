...
| `--exclude-route` | `build.exclude_routes` | — | (none) | Drop a prerendered route from the image, repeatable (`--exclude-route=/storybook`). Patterns are route paths, not file globs: a bare path covers the route **and its subtree** (`/admin` excludes `/admin/users` but not `/administration` — matching stops at a segment boundary), `*` matches within one segment and `**` across segments. Flag values and config values are merged, not overridden. **This filters prerendered output files, not the build**: the route's HTML (and any precompressed `.br`/`.gz`/`.zst` sibling) is removed, but its JavaScript chunks, imports and SBOM entries still ship — removing those needs the build-time filter, which is still open. A pattern matching no prerendered route **warns** rather than failing, since it means either a typo or a server-rendered route (compiled into the layered server bundle, where deleting a file cannot remove it) — either way the route is still in the image. Links from surviving pages into an excluded route also warn: the build continues, because removing the route is the point of the flag. |
