...
|---|---|---|---|---|
| 1 | Per-ref `pokkum.lock` slot for custom `--base` refs (every custom ref currently shares one `"custom"` lock key) | (A) key by `"custom:" + sha256(ref)[:12]`, mirroring how `distroless-node` got its own preset; needs a lockfile schema migration. (B) keep the current narrow guard (trust a `"custom"` entry only when `Ref`/`Digest` match) indefinitely | **A — approved by maintainer** | Decided, not implemented. Narrow wrong-image guard already shipped (`69914ac`, `internal/adapters/baseimage/resolver.go`); the shared-slot eviction behavior itself is unchanged. |
| 2 | `TrustedRootPath` (file path, `internal/ports/baseimage.go`) vs. the other two Sigstore trust-root consumers, which take bytes (`sigstore.Verifier`'s `trustedRootJSON`, the TUF refresh option) | (A) bridge `baseimage`'s path-based field with an internal temp-file write when bytes arrive from elsewhere. (B) change `ports.BaseImageRequest.TrustedRootPath` to bytes and have the CLI/caller read the file itself, matching the other two | **B — approved by maintainer** | Decided, not implemented. Found and deliberately left alone while wiring `--sigstore-tuf-refresh` (`eeaa83a`) — called out in that commit message as "a bigger call than belongs in this change." |
