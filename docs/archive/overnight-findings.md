...
**Status:** ✅ **Fixed** in `20ba1ec`, later the same day — this entry's original "not fixed, deliberately" text was correct when written and then went stale, which is itself an instance of the drift the generated docs now exist to prevent. All five mutating real-build tests copy their fixture into `t.TempDir()` first, generalising the pattern the runtime smoke test established, with `node_modules` symlinked so a per-test copy moves tens of kilobytes rather than ~69MB. Read-only tests were left alone, distinguished by evidence (the `sbom`, `packager` and `nativeinspect` packages contain no `os.WriteFile`/`MkdirAll` at all). Order-independence was proven at `-count=3 -shuffle=on` including the real-bun tests, with `git status --porcelain testdata/` clean afterwards, and generated fixture `pokkum.lock` files are now gitignored as belt-and-suspenders.
...
### 19. `pokkum metrics` serves nothing, while the feature list says it exposes a server

**Found by:** migrating `Feature-list.md` into the generated item source, which forced every claim to be checked against code before being carried forward.
**Where:** `cmd/pokkum/metrics.go`.
**What:** `Feature-list.md` claimed *"Exposes dedicated metrics scraping server via `pokkum metrics` (`--metrics-port`, default 8889)"*. The command parses `--metrics-port`, prints `Metrics pipeline ready. Press Ctrl+C to stop.`, and returns. Confirmed by grep: **no `ListenAndServe` and no `net.Listen` anywhere in the file**, and `UptimeSeconds` is hardcoded to `0`. Nothing binds a port; nothing can be scraped.
**Severity:** Medium as an overclaim — the command is advertised as a working observability endpoint and is not one. Low as a defect, since nothing else depends on it.
**Status:** recorded truthfully as the `problem` field on the `metrics-command-shape` item rather than migrated as a shipped feature, so the generated `Features.md` does not repeat the claim. Marked `awaiting-decision`, because `Roadmap.md` Tier 4 already asks whether this command should exist in this shape at all — the honest options are to implement a real server, fold the capability into build flags and document the port contract, or remove the command. That is a maintainer call, not a bug to patch.
**Note:** this is the fourth overclaim this migration surfaced, after Roadmap 2g (Go-version capture already wired), the node smoke test (shipped, still described as missing), and fixture isolation (fixed the same day it was logged as deferred). Forcing every claim through a schema that demands a proving `impl` path is what exposed them.

### 20. Two test packages authenticated against the developer's real Docker credentials and hung

**Found by:** the post-migration full-suite run, where `cmd/pokkum` and `internal/adapters/remotecacheutils` both took **~660 seconds** — the 10-minute `go test` timeout — rather than failing.
**Where:** neither package set `DOCKER_CONFIG`. `Cacher.Check` and several commands authenticate through `authn.DefaultKeychain`, which reads `$DOCKER_CONFIG/config.json`; with a credential store configured (mine is `credsStore: desktop`) that shells out to a `docker-credential-*` helper, which blocks indefinitely when Docker Desktop is not running or the keychain wants an interactive unlock. Goroutine dump confirmed it: `docker-credential-helpers/client.(*Shell).Output` under `authn/keychain.go`.
**Severity:** Medium, and the failure *mode* is the worst part — a hang, not an error. The suite burns ten minutes per affected package and produces no useful output, which is how it went unnoticed across earlier runs whose keychain state happened to answer quickly.
**The instructive part:** six packages already isolated `DOCKER_CONFIG`, and `internal/adapters/registry`'s own comment says this exact hang *"cost W6 a full debugging cycle"*. So the diagnosis was correct and the fix was applied to the six sites where the hang had been observed — leaving the two that had not yet been observed. Same shape as checklist row 18: a control applied to the noticed site rather than to every site with the same exposure. The right question after that first diagnosis was "which packages authenticate?", not "which packages hung?".
**Fixed:** `TestMain` in both packages points `DOCKER_CONFIG` at an empty temp dir, so `DefaultKeychain` falls back to anonymous auth — which is what the in-memory test registry expects anyway. `remotecacheutils` went from 662s to **0.6s**, `cmd/pokkum` to 8.6s.
