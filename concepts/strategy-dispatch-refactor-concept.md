...
2. **`packager`**: introduce the layer-building dispatch table, reusing `appendPrerenderedLayer` and the existing `build*Layer` helpers (`layer.go`) as-is — this refactor changes *how* they're selected, not their bodies.
