...
- **`--strategy=static` still has no runtime boot smoke test.** The layered strategy's new smoke test (above) doesn't cover static, since it ships a different PID 1 (`pokkum-static`) with no real fixture built for it yet. A newly-discovered, still-open gap — not urgent (static is a simpler runtime with far less to go wrong at boot than the layered strategy's Bun+adapter-node stack), but worth a follow-up fixture + smoke test for parity.
