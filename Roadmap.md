...
3. **Base Image Escrow / Mirroring — mirror writing done and verified; combining it with signature verification is confirmed broken.** Fail-closed mirror write error propagation and full test suite covering multi-arch index and single-image escrow, Cosign signature tag mirroring, lockfile integrity, mirror fallback, and air-gapped resolution. But resolving with both `--mirror-registry` and `VerifySignature: true` together — the expected combination for `distroless`/`chainguard`, which default `VerifySignature` on — now fails a real signature check with a docker-reference mismatch, confirmed by test, not yet fixed. See the dedicated note below.
...
**Status: mirror writing done and verified; mirror + signature verification confirmed broken together.** All mirror write errors (`remote.WriteIndex`, `remote.Write`, and Cosign `.sig` tag writes) are classified and fail-closed (`core.ErrPushFailed`/`core.ErrRegistryAuth`), preventing unwritten `MirrorRef` entries from being saved to `pokkum.lock`. Full test coverage in `internal/adapters/baseimage/resolver_test.go` verifies multi-platform indexes, single images, signature tag escrow, fail-closed write errors, graceful fallback to `PinnedRef` on mirror outage, air-gapped resolution from mirror, and stale mirror clearing on digest updates.

**But** a previously-unverified question is now confirmed broken by `TestResolve_EscrowMirror_VerifySignature_DockerReferenceMismatch`: static-key (and, via the same `checkSimpleSigningClaims` code path, keyless) Cosign verification requires the signature payload's embedded `docker-reference` to exactly match the repo being verified. A real upstream signature is only ever made for the upstream repo name, never the mirror's — so resolving with both `--mirror-registry` and `VerifySignature: true` (the default for `distroless`/`chainguard`) fails with a docker-reference mismatch, and this is not a rare outage-only edge case: mirror-resolve-first means the mirror is preferred whenever it's reachable, so this fails on *ordinary, successful* resolution, not just during an upstream outage. Fix needs to verify against the upstream repo recorded in the lockfile entry rather than the ref actually pulled from — not yet done.
