...
- [x] Standard OCI Annotations: `org.opencontainers.image.revision`/`.source`/`.version` auto-populate from git (commit SHA, remote URL, `git describe`) on every build by default — no flag needed, and no opt-out flag exists. `--image-label key=value` still works for explicit overrides (checked first, so an explicit value always wins) and any other annotation. `org.opencontainers.image.created` is not yet auto-populated from the commit timestamp. Outside a git repository (or without `git` on `PATH`), the build succeeds silently with those three labels simply absent — no warning is printed. (new flag: `--image-label key=value`, repeatable — matches `ko build --image-label`)
