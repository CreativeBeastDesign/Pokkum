...
### 1.1 🎯 Rolling-Deploy Asset Overlay (`--asset-overlay`)
...
**Why Pokkum specifically can solve this.** It already controls layer composition *and* already records `pokkum.dev/image-history` (used by `pokkum rollback`). Read the previous N generations' digests from that history, pull each one's `/app/client/_app/immutable` content by digest, and merge non-conflicting hashed files into a **separate overlay layer**. Because the merged bytes are byte-identical to what a node already pulled for the prior generation, the layer dedupes at the registry and on the node — cost is near zero.

**Design (from `AdditionalFeatures.md`, unresolved as of this writing):**
- The overlay's source digests **must** join the composite input hash — a cache hit that serves a stale overlay is worse than no overlay.
- Conflict policy when the same hashed path carries different bytes across generations: content hashing should make this impossible, which argues for a **hard build failure**, not a silent pick — a collision means an upstream invariant broke.
- Interaction with `pokkum verify --rebuild`: the overlay makes an image's bytes depend on its *predecessors*, so rebuild-verification needs the same history to reproduce it. This needs an explicit answer *before* implementation, not discovered during it.

**Flags/Interface:** `--asset-overlay=<n>` on `pokkum build` (default `0` = off, preserving current behavior).
...
| - | ---- | --------------- | --------------- |
| 2a | KMS-backed signing | `POKKUM_SIGNING_KEY` holding a raw private key in an env var is a smell. Support `awskms://`, `gcpkms://`, `azurekms://`, `hashivault://`, and PKCS#11 URIs. | Raw-key-in-env-var is the exact pattern a security review flags first. |
...
| 2d | Multi-arch attestation subject | Document, test, and pin whether `.sig`/`.att` attach to the **index** digest, the **per-platform manifest** digests, or both. Verifiers differ — getting it wrong means policy passes under `cosign verify` and fails under Kyverno (or the reverse). | Attaching to both, and documenting which one `pokkum verify` checks, is the safe answer — currently unstated. |
