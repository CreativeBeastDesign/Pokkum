...
- [x] **Composite Remote OCI Input Caching**: Skip builds entirely (sub-100ms) if an image already exists in the registry for a given hash of `(source + lockfile + baseDigest + bunVersion)`. This originally shipped with several confirmed, security-relevant gaps: (1) a cache hit was trusted on tag existence alone, with no signature/SBOM/annotation check, so anyone with push access to a repo's `cache-<hash>` tags — routinely a narrower credential than release-tag push authority — could get arbitrary content promoted straight to the release tags; (2) the input hash omitted most image-determining build flags (signing, the entire runtime contract, telemetry, remaining compile/SBOM options), so two meaningfully different builds could collide on the same cache tag; (3) the source-tree/lockfile hash folded raw file content directly after a NUL-delimited path with no length framing, which is forgeable (a constructed second-preimage collision proved this); (4) a cache hit skipped SBOM attachment and signing entirely, so a signed build's own attestation could be silently skipped. Fixed: the tree/lockfile hash now frames each entry with a fixed-length content digest instead of raw content; the input hash now covers the runtime and telemetry contracts plus every other previously-missing flag; a `ReconcileTags` failure is no longer discarded (`Hit: false` propagates instead of a false success); and — the most consequential change — the cache is now only ever consulted when `Sign` is false, so a signed build always runs a fresh build and a fresh sign rather than ever adopting a previously-cached digest. This does not make the cache forgery-proof for unsigned builds (tag-existence trust remains a residual gap there — closing it fully would mean verifying a real signature on the cache-hit image before reconciling tags, not yet attempted), but it closes the concrete escalation from cache-tag push access to release-tag promotion for every build that actually attests anything. See `internal/adapters/remotecacheutils/remotecacheutils_test.go` for the adversarial regression tests (`TestComputeSourceTreeHash_FramingCannotBeForged`, `TestComputeInputHash_CoversPreviouslyMissingFields`, `TestCacher_Check_ReconcileTagsFailureIsNotReportedAsHit`) and `internal/core/pipeline_test.go`'s `TestBuildPushSuccess_SignedBuildNeverConsultsRemoteCache`.
