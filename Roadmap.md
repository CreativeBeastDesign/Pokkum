...
- [x] Zero-Config Auto-Injection: Auto-injecting the adapter and `SOURCE_DATE_EPOCH` pinning without manual `svelte.config.js` edits (supported via `--inject` and `--no-inject` flags). (see [pokkum-injection-concept.md](concepts/archive/pokkum-injection-concept.md))
...
- [ ] **Static end-to-end integration fixture**: add an automated integration test that builds a genuinely fully-prerendered SvelteKit app with `--static`, then asserts on the resulting image that (a) it has no Bun/supervisor layer, (b) the base is `chainguard/static`, and (c) `pokkum-static` serves HTML with working Range/ETag/Content-Encoding against the `.gz/.br/.zst` sidecars — locking in the `.svelte-kit/output` staging path beyond the current unit tests.
- [ ] **Adapter-node handler-version hardening**: the layered prerendered-path patch (`bunexec/prerendered_patch.go`) replaces a specific `path.join(dir, "prerendered")` idiom and warn-and-continues if a different `@sveltejs/adapter-node` minor version restructures `handler.js`. Verify against both v3 and v5 outputs and tighten the matcher so an unpatched build can't silently ship 404-ing prerendered pages.
- [ ] **SPA-fallback decision for `adapter-static`**: when a static site uses a `fallback` page (SPA mode), decide and document whether `pokkum-static` should serve the fallback for unknown routes on a purely static image, and implement + test the chosen behavior.
...
- **Solution** (see `concepts/archive/pokkum-layer-caching-concept.md` §5.2 for full option analysis):
  - **Option A — compiled stub launcher**: ship a tiny `bun build --compile` executable whose only code is `await import("/app/server/index.js")`, path compiled in rather than read from argv/env. Fully closes the `BUN_BE_BUN`/`bunx` surface; CLI parity with v0.1's exe strategy. Needs a spike to confirm compiled exes can runtime-import external files including transpilation.
  - **Option C — supervisor startup attestation**: bake a SHA-256 manifest of `/app` into the `/pokkum/init` layer at build time; `pokkum-init` verifies it before exec and refuses to start on mismatch. Restores tamper-evidence without depending on cluster-level readonly-fs policy. Cheap (~100 LOC per the concept doc's estimate), no runtime overhead after startup.
