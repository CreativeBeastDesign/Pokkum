...
func TestRenderSPDXJSON_Deterministic(t *testing.T) {
	...
	id := contentIdentityUUID("app", "1.0.0", pkgs, "", "", scannerutils.DistroInfo{}, false, 0)

	out1, err := renderSPDXJSON("app", "1.0.0", id, createdAt, pkgs, "", "", scannerutils.DistroInfo{}, false, 0)
	if err != nil {
		t.Fatal(err)
	}
	out2, err := renderSPDXJSON("app", "1.0.0", id, createdAt, pkgs, "", "", scannerutils.DistroInfo{}, false, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderCycloneDXJSON_Deterministic(t *testing.T) {
	...
	out, err := renderCycloneDXJSON("app", "1.0.0", id, createdAt, pkgs, "", "", scannerutils.DistroInfo{}, false, 0)
	if err != nil {
		t.Fatal(err)
	}
	var doc map[string]any
...
func TestContentIdentityUUID_Deterministic(t *testing.T) {
	...
	id1 := contentIdentityUUID("app", "1.0.0", pkgs, "", "", scannerutils.DistroInfo{}, false, 0)
	id2 := contentIdentityUUID("app", "1.0.0", []scannerutils.CatalogPackage{pkgs[1], pkgs[0]}, "", "", scannerutils.DistroInfo{}, false, 0)
	if id1 != id2 {
		t.Errorf("contentIdentityUUID depends on package slice order: %v != %v", id1, id2)
	}

	id3 := contentIdentityUUID("app", "1.0.1", pkgs, "", "", scannerutils.DistroInfo{}, false, 0)
	if id1 == id3 {
		t.Error("contentIdentityUUID did not change when the version changed")
	}

	id4 := contentIdentityUUID("app", "1.0.0", pkgs, "1.2.2", "abc123", scannerutils.DistroInfo{}, false, 0)
	if id1 == id4 {
...
func TestRenderCycloneDXJSON_BunComponent(t *testing.T) {
	createdAt := time.Unix(1700000000, 0).UTC().Format(time.RFC3339)
	id := contentIdentityUUID("app", "1.0.0", nil, "1.2.2", "deadbeef", scannerutils.DistroInfo{}, false, 0)

	out, err := renderCycloneDXJSON("app", "1.0.0", id, createdAt, nil, "1.2.2", "deadbeef", scannerutils.DistroInfo{}, false, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderSPDXJSON_BunComponent(t *testing.T) {
	...
	out, err := renderSPDXJSON("app", "1.0.0", id, createdAt, nil, "1.2.2", "deadbeef", scannerutils.DistroInfo{}, false, 0)
	if err != nil {
		t.Fatal(err)
	}
