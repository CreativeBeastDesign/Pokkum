...
func freshDecode(t *testing.T, path string) []byte {
	...
	if err != nil {
		t.Skipf("%s not embedded; run `make supervisor` first", path)
	}
	data, err := embeddedbinaryutils.Decode(embeddedbinaryutils.NewDecoder(), compressed, errSupervisorCorrupt)
	if err != nil {
...
func TestBinaryDoesNotCrossPlatforms(t *testing.T) {
	...
	if err != nil {
		if errors.Is(err, core.ErrSupervisorUnavailable) {
			t.Skip("supervisor binaries not embedded; run `make supervisor` first")
		}
...
// TestSupervisorBlobsCoverEverySupportedPlatform proves the memoisation table
// has no gap: every path Binary can produce is a table entry, so
// loadSupervisorBlob's uncached fallback is unreachable for real platforms. It
// asserts this by checking the table directly rather than by trusting the
// switch and the table to have been edited together.
func TestSupervisorBlobsCoverEverySupportedPlatform(t *testing.T) {
	for _, plat := range ports.SupportedPlatforms {
		...
		default:
			t.Fatalf("platform %s is in ports.SupportedPlatforms but has no supervisor asset constant", plat)
		}
		if _, ok := supervisorBlobs[want]; !ok {
			t.Errorf("asset %s (for %s) has no memoisation entry; Binary would decompress it on every call", want, plat)
		}
	}
	if len(supervisorBlobs) != len(ports.SupportedPlatforms) {
		t.Errorf("memoisation table holds %d entries for %d supported platforms",
			len(supervisorBlobs), len(ports.SupportedPlatforms))
	}
}

// TestLoadSupervisorBlobUnknownPath covers the fallback branch: a path with no
// table entry is read uncached and reports the read failure rather than
// panicking on a nil map value.
func TestLoadSupervisorBlobUnknownPath(t *testing.T) {
	res := loadSupervisorBlob("bin/pokkum-init-linux-sparc64.zst")
	if res.readErr == nil {
		t.Fatal("loadSupervisorBlob on an unembedded path did not report a read error")
	}
	if res.data != nil {
		t.Error("loadSupervisorBlob returned data for an unembedded path")
	}
}
...
func TestBinaryConcurrentCallsShareOneBlob(t *testing.T) {
	...
	p := New(slog.Default())
	if _, err := p.Binary(context.Background(), ports.LinuxAMD64); errors.Is(err, core.ErrSupervisorUnavailable) {
		t.Skip("supervisor binaries not embedded; run `make supervisor` first")
	}
